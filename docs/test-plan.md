...
- `/api/orders` — creates order row in Neon Postgres and returns customer status token plus approximate ETA
- `/api/orders/[statusToken]` — reads safe customer-facing order status by random token
- `/api/agent/orders/[updateToken]` — validates shared PIN, validates status transition, and updates the Neon order row
...
- Checkout shows retry/support fallback if Neon insert times out or returns an invalid response
...
- Neon timeout maps to retryable checkout error
- Neon malformed/unexpected row maps to non-success state, never false confirmation
- Database row with unknown status maps to safe error state, not a broken UI
...
- Complete customer checkout through delivered status with mocked Neon order repository
...
- Neon order repository maps success, timeout, constraint errors, missing row, duplicate token, and update failure
