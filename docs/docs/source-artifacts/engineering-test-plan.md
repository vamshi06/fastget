...
Branch: main
Repo: Fastget standalone app repo, to be created
...
## Critical Paths
- E2E: complete customer checkout through delivered status with mocked Apps Script adapter.
- E2E: checkout backend failure shows retry/support and does not show confirmation.
- E2E: wrong agent PIN cannot update order status.
- Unit: order status state machine legal transitions and rejected transitions.
- Unit: order validation and pay-on-delivery checkout payload shape.
- Unit: Apps Script adapter maps success, timeout, non-200, malformed JSON, missing row, duplicate row, and update failure.
- Unit: price/convenience fee calculation for typical Rs. 200 AOV order and boundary values.
